...
if ( ! function_exists( 'hotelgalaxy_icon_callout_create' ) ) {
	...
	function hotelgalaxy_icon_callout_create() {
		...
		print_r(get_theme_mod('footer_collout_icon_4'));	
						...
						<?php for( $i=1; $i<=4; $i++ ){
							...
							<div id="footer-icon-callout-<?php echo esc_attr($i)?>" class="col-md-3 col-sm-6 col-xs-12">							
								<span class="icon-callout-inner-icon">
									<i class="fa <?php echo isset($hotelgalaxy_settings['footer_collout_icon_'.$i]) ? esc_attr( $hotelgalaxy_settings['footer_collout_icon_'.$i] ) : '' ?>"></i>
								</span>

								<span class="icon-callout-inner-title">
									<?php echo  isset($hotelgalaxy_settings['footer_collout_title_'.$i]) ? esc_html( $hotelgalaxy_settings['footer_collout_title_'.$i] ) : '' ?>
								</span>
...
if ( ! function_exists( 'hotelgalaxy_back_to_top_control' ) ) {
	...
	function hotelgalaxy_back_to_top_control() {		
		...
		if (  hotelgalaxy_get_option('back_to_top') !== 'enable') {
			return;
		}

		echo apply_filters('hotelgalaxy_back_to_top_output',
			sprintf( '<a href="javascript:void(0);" class="scroll-top" rel="nofollow"><i class="%1$s"></i></a>', 
