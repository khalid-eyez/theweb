...
class Hotel_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'Hotelgalaxy-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'hotelgalaxy_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'hotelgalaxy_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$hotelgalaxy_customizer_notify_show_recommended_actions = get_option( 'hotelgalaxy_customizer_notify_show' );

		foreach ( $hotelgalaxy_customizer_notify_recommended_actions as $key => $hotelgalaxy_lite_customizer_notify_recommended_action ) {

			if ( $hotelgalaxy_customizer_notify_show_recommended_actions[ $hotelgalaxy_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $hotelgalaxy_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$hotelgalaxy_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $hotelgalaxy_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {

				$active = $this->check_active( $hotelgalaxy_customizer_notify_recommended_actions['plugin_slug'] );

				$hotelgalaxy_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $hotelgalaxy_lite_customizer_notify_recommended_action['plugin_slug'] );

				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$hotelgalaxy_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$hotelgalaxy_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
					break;
					case 'activate':
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
					break;
					case 'deactivate':
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
					$hotelgalaxy_lite_customizer_notify_recommended_action['button_label'] = $hotelgalaxy_deactivate_button_label;
					break;
				}
			}
			$formatted_array[] = $hotelgalaxy_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$hotelgalaxy_lite_customizer_notify_show_recommended_plugins = get_option( 'hotelgalaxy_customizer_notify_show_recommended_plugins' );

		foreach ( $hotelgalaxy_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $hotelgalaxy_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $hotelgalaxy_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				$ti_customizer_notify_recommended_plugin['button_label'] = $hotelgalaxy_deactivate_button_label;
				break;
			}
		...
		$json['total_actions']       = count( $hotelgalaxy_customizer_notify_recommended_actions );
	...
	protected function render_template() {
		...
		<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
			...
			<h3 class="accordion-section-title">
				...
				<# if( data.recommended_actions.length > 0 ){ #>
				<span class="hotelgalaxy-customizer-plugin-notify-actions-count">
					<span class="current-index">{{ data.recommended_actions[0].index }}</span>
					{{ data.total_actions }}
				</span>
				<# } #>
			</h3>
			<div class="hotelgalaxy-theme-recomended-actions_container" id="plugin-filter">
				<# if( data.recommended_actions.length > 0 ){ #>
				<# for (action in data.recommended_actions) { #>
				<div class="hotelgalaxy-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
					<# if( !data.recommended_actions[action].check ){ #>
					<div class="hotelgalaxy-epsilon-recommeded-actions">
						<p class="title">{{ data.recommended_actions[action].title }}</p>
						<span data-action="dismiss" class="dashicons dashicons-no hotelgalaxy-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
				...
				<div class="hotelgalaxy-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
					<# if( !data.recommended_plugins[action].check ){ #>
					<div class="hotelgalaxy-epsilon-recommeded-actions">
						<p class="title">{{ data.recommended_plugins[action].title }}</p>
						<span data-action="dismiss" class="dashicons dashicons-no hotelgalaxy-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
